...
#randomizeSourcePool(phy=sim_tre, com = com.data.a, sourcePool = "com1", N =5)
#randomizeSourcePool(phy=sim_tre, com = com.data.a, sourcePool = "com2", N =5)

#######  Compare to picante output
#ses.mpd.sourcePool(phy=sim_tre, com = com.data.a, sourcePool = "com1", N =100)
#ses.mpd(samp = com.data.a, dis=cophenetic(sim_tre), null.model = "sample.pool", runs =100)

randomizeSourcePool <- function(phy, com, sourcePool, N){
  ...
  samp.prob <- (sourcePoolCounts/orig.com.counts)
  ...
  ### Randomization  
    ...
    while (rep < N){
      ...
      for (i in 1:nrow(orig.com)){ # for each species 
        sp.prob <- samp.prob[rownames(orig.com)[i]] # the sample probability for one species
        # draw occurence in community | probability of sampling in source pool
        sim.sp.occ <- (sample(c(0,1), size=length(orig.com[i,]), replace=T, 
                              prob = c(1-sp.prob, sp.prob)))
        #print(sim.sp.occ)
        sim.comm.df <- rbind(sim.comm.df, sim.sp.occ)
      }
      sim.comm.df
      colnames(sim.comm.df) = colnames(orig.com) 
      rownames(sim.comm.df) = rownames(orig.com) 
...
ses.mpd.sourcePool <- function(phy, com, sourcePool, N){

  mpd.obs.all <-  mpd(samp = com, dis=cophenetic(phy))
  rowSums(as.data.frame(!com==0))
  
  rand.com.tmp <- randomizeSourcePool(phy =phy, com = com, sourcePool = sourcePool, N =N)
  colSums(as.data.frame((rand.com.tmp)[1]))
  rowSums(as.data.frame((rand.com.tmp)[1]))
  null.mpd <- lapply(1:length(rand.com.tmp), function(x) mpd(rand.com.tmp[[x]], cophenetic(phy)))
  null.mpd.c <- as.data.frame(do.call(rbind, null.mpd))
  colnames(null.mpd.c) <- rownames(com)
  
  output.df <- data.frame()
  ## For each community:
  for (i in 1:ncol(null.mpd.c)){
    
    ntaxa <- rowSums(!com==0)[i] #number of species
    mpd.obs <- mpd.obs.all[i] #observed mean metric
...
ses.mntd.sourcePool <- function(phy, com, sourcePool, N){
  
  mntd.obs.all <-  mntd(samp = com, dis=cophenetic(phy))
  
  rand.com.tmp <- randomizeSourcePool(phy =phy, com = com, sourcePool = sourcePool, N =N)
  null.mntd <- lapply(1:length(rand.com.tmp), function(x) mntd(rand.com.tmp[[x]], cophenetic(phy)))
  null.mntd.c <- as.data.frame(do.call(rbind, null.mntd))
  colnames(null.mntd.c) <- rownames(com)
  ...
  for (i in 1:ncol(null.mntd.c)){
    ...
    ntaxa <- rowSums(!com==0)[i] #number of species
    mntd.obs <- mntd.obs.all[i] #observed mean metric
