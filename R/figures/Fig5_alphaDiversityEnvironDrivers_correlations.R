...
############# correlation of SES : Environmental variables  #########################################################
...
alpha.div.master.env.pca <- merge(alpha.div.master.env, env.pca, by.x=1, by.y=0)
...
corfun<-function(x, y) {
  corr=(cor.test(x, y, alternative="two.sided", method="spearman")) 
}
...
env_correlate_plot <- ggplot(env_correlate, aes(y=measure, x=clade, fill=rho.est)) +
  ...
  scale_fill_gradient2(low="#44AA77", high="#AA4488", mid="beige", na.value="white", limits=c(-.9, .9)) + # darkgreen, violet
  ...
  coord_fixed(ratio=1)
ggsave("figs/Figure5_alpha_env_correlates.pdf", env_correlate_plot, width = 8.5, height = 11)
