...
source(file="R/functions/chooseClade.R")
...
################################ Summits Species Pool: LGM community ################################ 

persistent.sperm.summitsPool.LGM.summary <- read.csv("output/8_PhyoDiversity/alpha/dynamic/speciesPools/2_allSummits/LGM/output/Spermatophyta.persis.summary_table.csv")
persistent.sperm.summitsPool.LGM.summary
persistent.sperm.summitsPool.LGM.summary_tab <- t(as.data.frame(persistent.sperm.summitsPool.LGM.summary[,3], strings.as.factors=F))
colnames(persistent.sperm.summitsPool.LGM.summary_tab) <- t(persistent.sperm.summitsPool.LGM.summary)[2,]
rownames(persistent.sperm.summitsPool.LGM.summary_tab) <- NULL
persistent.sperm.summitsPool.LGM.summary_tab <- as.data.frame(cbind(summit = "Persistent", clade = "Spermatophyta", persistent.sperm.summitsPool.LGM.summary_tab, pool="All Summits"))

persistent.ast.summitsPool.LGM.summary <- read.csv("output/8_PhyoDiversity/alpha/dynamic/speciesPools/2_allSummits/LGM/output/asterales.persis.summary_table.csv")
persistent.ast.summitsPool.LGM.summary
persistent.ast.summitsPool.LGM.summary_tab <- t(as.data.frame(persistent.ast.summitsPool.LGM.summary[,3], strings.as.factors=F))
colnames(persistent.ast.summitsPool.LGM.summary_tab) <- t(persistent.ast.summitsPool.LGM.summary)[2,]
rownames(persistent.ast.summitsPool.LGM.summary_tab) <- NULL
persistent.ast.summitsPool.LGM.summary_tab <- as.data.frame(cbind(summit = "Persistent", clade = "Asterales", persistent.ast.summitsPool.LGM.summary_tab, pool="All Summits"))

persistent.poa.summitsPool.LGM.summary <- read.csv("output/8_PhyoDiversity/alpha/dynamic/speciesPools/2_allSummits/LGM/output/poales.persis.summary_table.csv")
persistent.poa.summitsPool.LGM.summary
persistent.poa.summitsPool.LGM.summary_tab <- t(as.data.frame(persistent.poa.summitsPool.LGM.summary[,3], strings.as.factors=F))
colnames(persistent.poa.summitsPool.LGM.summary_tab) <- t(persistent.poa.summitsPool.LGM.summary)[2,]
rownames(persistent.poa.summitsPool.LGM.summary_tab) <- NULL
persistent.poa.summitsPool.LGM.summary_tab <- as.data.frame(cbind(summit = "Persistent", clade = "Poales", persistent.poa.summitsPool.LGM.summary_tab, pool="All Summits"))

persistent.lam.summitsPool.LGM.summary <- read.csv("output/8_PhyoDiversity/alpha/dynamic/speciesPools/2_allSummits/LGM/output/lamiales.persis.summary_table.csv")
persistent.lam.summitsPool.LGM.summary
persistent.lam.summitsPool.LGM.summary_tab <- t(as.data.frame(persistent.lam.summitsPool.LGM.summary[,3], strings.as.factors=F))
colnames(persistent.lam.summitsPool.LGM.summary_tab) <- t(persistent.lam.summitsPool.LGM.summary)[2,]
rownames(persistent.lam.summitsPool.LGM.summary_tab) <- NULL
persistent.lam.summitsPool.LGM.summary_tab <- as.data.frame(cbind(summit = "Persistent", clade = "Lamiales", persistent.lam.summitsPool.LGM.summary_tab, pool="All Summits"))

persistent.caryo.summitsPool.LGM.summary <- read.csv("output/8_PhyoDiversity/alpha/dynamic/speciesPools/2_allSummits/LGM/output/caryophyllales.persis.summary_table.csv")
persistent.caryo.summitsPool.LGM.summary
persistent.caryo.summitsPool.LGM.summary_tab <- t(as.data.frame(persistent.caryo.summitsPool.LGM.summary[,3], strings.as.factors=F))
colnames(persistent.caryo.summitsPool.LGM.summary_tab) <- t(persistent.caryo.summitsPool.LGM.summary)[2,]
rownames(persistent.caryo.summitsPool.LGM.summary_tab) <- NULL
persistent.caryo.summitsPool.LGM.summary_tab <- as.data.frame(cbind(summit = "Persistent", clade = "Caryophyllales", persistent.caryo.summitsPool.LGM.summary_tab, pool="All Summits"))
...
SES_all <- rbind(summit.ast.EcrinsPool.summary_tab, summit.poa.EcrinsPool.summary_tab, summit.lam.EcrinsPool.summary_tab, summit.cary.EcrinsPool.summary_tab,
                 ...
                 persistent.sperm.summitsPool.LGM.summary_tab, persistent.ast.summitsPool.LGM.summary_tab, persistent.poa.summitsPool.LGM.summary_tab, persistent.lam.summitsPool.LGM.summary_tab, persistent.caryo.summitsPool.LGM.summary_tab,
...
head(ses.master)
levels(ses.master$summits)[levels(ses.master$summits)=="Persistent"] <- "LGM"
ses.master$summits <- factor(ses.master$summits, levels = c("Brevoort", "Pelvoux", "Choisy", "La Meije",
                                                            "Burlan","Occidentale Ailefroide", "Plat de la Selle",
                                                            "Rouies","Barre des Ecrins", "Olan", "Muraillette",
                                                            "Sirac" ,  "Rateau" , "Lauvitel", "Rocher de la Selle",
                                                            "LGM", "All Summits"))                                                       
...
ses.rd.damo <- ggplot(ses.master, aes(y=clade, x=summits, fill=as.numeric(as.character(obs.z))))
...
## Plot SES mntd/mpd for each summit, each clade, randomization = "phylogeny.pool"
ses.damo <- ggplot(ses.master.damo, aes(y=clade, x=summits, fill=as.numeric(as.character(obs.z))))
