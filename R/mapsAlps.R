#####################################################################################################################
############# Map the Ecrins National Park, France, and each summit sampled #########################################
############# Hannah E. Marx, 17 Mar 2016 ########################################################################### 
#####################################################################################################################
### Referenced: http://www.nickeubank.com/wp-content/uploads/2015/10/RGIS3_MakingMaps_part1_mappingVectorData.html

source("R/addScaleBarMap.R") ## add scale bar to map
...
layerName <- "ecrinsNP"  
data_projected <- readOGR(dsn="output/10_Maps/ecrinsNP/", layer=layerName) 
...
pdf(file="output/10_Maps/FranceMapSummitsColor.pdf") 
p <- ggplot()
...
layer <- stack("output/10_Maps/EcrinsBaseZoom.tif") #EPSG: 3857
...
# et voila!
ggsave("output/10_Maps/EcrinsSummitsColor.pdf", width = 8, height = 8)
...
pdf(file="output/10_Maps/EcrinsSummitsTopoMNTD.pdf")
ggmap(ecrins) + 
...
pdf(file="output/10_Maps/EcrinsSummitsTopoMPD.pdf")
ggmap(ecrins) + 
