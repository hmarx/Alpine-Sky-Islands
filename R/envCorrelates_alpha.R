### Coorelation of Environmental variables ~ alpha PD)
...
#write.csv(env_correlate, file="output/10_Environment/alpha/alpha_hyp_correlates.csv")
head(env_correlate)
str(env_correlate)
...
env_correlate_plot <- ggplot(env_correlate, aes(y=measure, x=clade, fill=rho.est)) +
  ...
  coord_fixed(ratio=1)
ggsave("output/10_Environment/alpha/alpha_env_correlates.pdf", env_correlate_plot, width = 8.5, height = 11)
