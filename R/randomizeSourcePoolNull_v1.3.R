...
ses.mntd.sourcePool(phy=sim_tre, com = com.data.a, sourcePool = "com1", N =100)
...
randomizeSourcePool <- function(phy, com, sourcePool, N){
  ...
  ### Randomization  
    ...
    while (rep < N){
      ...
      sim.comm.df
      colnames(sim.comm.df) = colnames(focal.com) 
...
ses.mpd.sourcePool <- function(phy, com, sourcePool, N){
  ...
  for (i in 1:ncol(null.mpd.c)){
    ntaxa <- rowSums(!t(focal.com)==0)[i] #number of species
    mpd.obs <- mpd.obs.all[colnames(null.mpd.c[i])] #observed mean metric
...
ses.mntd.sourcePool <- function(phy, com, sourcePool, N){
  ...
  for (i in 1:ncol(null.mntd.c)){
    ...
    ntaxa <- rowSums(!t(focal.com)==0)[i] #number of species
    mntd.obs <- mntd.obs.all[colnames(null.mntd.c[i])] #observed mean metric
