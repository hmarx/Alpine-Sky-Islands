...
ggplot(master.ses, aes(x = value, fill = variable)) + 
  ...
  ggtitle("Summit Community, Ecrins Source Pool") + 
  scale_alpha_discrete(range = c("RD" = 1, "DAMOCLES" = .7), guide=FALSE) +
  scale_x_continuous(limits = c(-5,5)) + 
  scale_y_continuous(limits = c(0, .25)) +
  theme(strip.text.x = element_text(size = 12, face="bold"))
#dev.off()
ggsave(filename="output/9_PhyoDiversity/Spermatophyta/dynamic/cladesEachSummit_EcrinsPool/summits/SES.all.density.summits.pdf")
ggsave(filename="figs/Figure3a_SES.all.density.summits.pdf")


pdf(file="output/9_PhyoDiversity/Spermatophyta/dynamic/cladesEachSummit_EcrinsPool/summits/SES.all.density.green.pdf")
ggplot(master.ses, aes(x = value, fill = variable)) + 
