...
pezAlpes.summits <- comparative.comm(phy = alps.phy, comm = summits.sites[-c(1, 4),]) # Remove: Ecrins NP, Persistent
pezAlpes.summits # 215  taxa

tmp2 <- pezAlpes.summits$comm
tmp2[which(tmp2 != 0)] <- 1
alps.summits.pa.all <- t(tmp2)
alps.summits.pa.all <- cbind(row.names(alps.summits.pa.all), alps.summits.pa.all)
