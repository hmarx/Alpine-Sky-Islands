#####################################################################################################################
############# Generates a distribution of SES for each summit ################################################## 
############# Used to compare observed regression of SES ~ environmental variables to null distribution 
############# Calulations were made on a server; with TNRS taxized taxonomy dataset (shouldn't change at all for the updated TaxRef taxonomy)
...
for (i in 1:99){
  ...
  #write.csv(ecrins.sesmpd.phylonull, file="output/ecrins.sesmpd.phylonull.csv")
  #write.csv(ecrins.sesmntd.phylonull, file="output/ecrins.sesmntd.phylonull.csv")
  ...
  #write.csv(summits.sesmpd.phylonull, file="output/summits.sesmpd.phylonull.csv")
  #write.csv(summits.sesmntd.phylonull, file="output/summits.sesmntd.phylonull.csv")
  ...
  #write.csv(persistent.sesmpd.phylonull, file="output/persistent.sesmpd.phylonull.csv")
  #write.csv(persistent.sesmntd.phylonull, file="output/persistent.sesmntd.phylonull.csv")
  ...
  #write.csv(phylogeny.poolSES, file="output/phylogeny.pool.SES.csv")
  ...
  #write.csv(summit.poolSES, file="output/summit.pool.SES.csv")
  ...
  #write.csv(persistent.poolSES, file="output/persistent.pool.SES.csv")
  
  master.ses.alpha <- rbind(phylogeny.poolSES, summit.poolSES, persistent.poolSES)
  head(master.ses.alpha)
  write.csv(master.ses.alpha, file=paste("output/ses.static.nullCom.", i, ".csv", sep=""))
