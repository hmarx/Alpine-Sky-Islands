...
write.csv(obs.mntd.aster, file="obs.mntd.persis.aster.csv")
write.csv(obs.mpd.aster, file="obs.mpd.persis.aster.csv")

damo.bs.aster <- DAMOCLES_bootstrap(
  phy = alps.damocles.aster$phy,
  pa = alps.damocles.aster$data[,c(1, 10)],  #persistent P/A,
...
write.csv(damo.bs.aster$null_community_data, file="asterales.persis.null_community_data.csv")
write.csv(damo.bs.aster$summary_table, file="asterales.persis.summary_table.csv")
